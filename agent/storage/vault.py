...
class VaultManager:
    ...
    def list_history(self) -> List[Dict[str, Any]]:
        """Return all historical research reports whose files exist."""
        history = self._load_history()
        valid = [item for item in history if Path(item.get("filepath", "")).exists()]
        if len(valid) != len(history):
            self._save_history(valid)
        return valid
...
<body>
    <div class="no-print" style="margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center;">
        <button onclick="window.print()" style="padding: 8px 16px; background: #2b6cb0; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: 600;">Print / Save as PDF</button>
